...
if [ -n "${UV_DOWNLOAD_URL:-}" ]; then
    ...
    ARTIFACT_DOWNLOAD_URL="${INSTALLER_BASE_URL}/astral-sh/uv/releases/download/0.9.18"
fi
...
# https://ghfast.top/https://github.com//astral-sh/uv/issues/6965#issuecomment-2915796022
get_home() {
    if [ -n "${HOME:-}" ]; then
        echo "$HOME"
    elif [ -n "${USER:-}" ]; then
...
https://ghfast.top/https://github.com//astral-sh/uv/releases/download/0.9.18
...
downloader() {
    ...
    # https://ghfast.top/https://github.com//boukendesho/curl-snap/issues/1
    _snap_curl=0
    ...
    then
      ...
      say "See https://ghfast.top/https://github.com//boukendesho/curl-snap/issues/1"
      exit 1
